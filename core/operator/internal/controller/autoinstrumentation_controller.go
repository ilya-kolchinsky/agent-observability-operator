...
func (r *AutoInstrumentationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if r.Log.GetSink() != nil {
		logger = r.Log.WithValues("autoInstrumentation", req.NamespacedName)
	}

	var demo platformv1alpha1.AutoInstrumentation
	...
	logger.Info(
		"reconciling AutoInstrumentation for end-to-end telemetry path",
	...
	if statusChanged {
		...
		logger.Info(
			"updated AutoInstrumentation status after reconciliation",
	...
	logger.Info(
		"AutoInstrumentation status already up to date",
		"phase", demo.Status.Phase,
		"generatedInstrumentationName", demo.Status.GeneratedInstrumentationName,
		"generatedConfigMapName", demo.Status.GeneratedConfigMapName,
	)
...
func (r *AutoInstrumentationReconciler) reconcileInstrumentation(ctx context.Context, logger logr.Logger, desiredInstrumentation *otelv1alpha1.Instrumentation) error {
...
func (r *AutoInstrumentationReconciler) reconcileConfigMap(ctx context.Context, logger logr.Logger, desiredConfigMap *corev1.ConfigMap) error {
...
func buildDesiredInstrumentation(demo *platformv1alpha1.AutoInstrumentation, namespace string, resolvedSpec *platformv1alpha1.InstrumentationSpec) *otelv1alpha1.Instrumentation {
...
func buildDesiredRuntimeCoordinatorConfigMap(demo *platformv1alpha1.AutoInstrumentation, namespace string, resolvedSpec *platformv1alpha1.InstrumentationSpec) *corev1.ConfigMap {
...
func renderRuntimeCoordinatorConfig(demo *platformv1alpha1.AutoInstrumentation, namespace string, resolvedSpec *platformv1alpha1.InstrumentationSpec) string {
...
func prepareDeploymentForInstrumentation(demo *platformv1alpha1.AutoInstrumentation, deployment *appsv1.Deployment, instrumentation *otelv1alpha1.Instrumentation, configMapName string) (bool, error) {
...
func desiredContainerEnvVars(demo *platformv1alpha1.AutoInstrumentation, workloadNamespace, configMapName string) []corev1.EnvVar {
	serviceName := desiredServiceName(demo)
	collectorEndpoint := collectorEndpointForDemo(demo)
	collectorTracesEndpoint := collectorTracesEndpointForDemo(demo)
...
func desiredServiceName(demo *platformv1alpha1.AutoInstrumentation) string {
...
func collectorEndpointForDemo(demo *platformv1alpha1.AutoInstrumentation) string {
...
func collectorTracesEndpointForDemo(demo *platformv1alpha1.AutoInstrumentation) string {
	endpoint := collectorEndpointForDemo(demo)
...
func (r *AutoInstrumentationReconciler) SetupWithManager(mgr ctrl.Manager) error {
	if r.Log.GetSink() == nil {
		r.Log = ctrl.Log.WithName("controllers").WithName("AutoInstrumentation")
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&platformv1alpha1.AutoInstrumentation{}).
		Named("autoinstrumentation").
		Complete(r)
}
