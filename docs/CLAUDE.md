...
1. **Control plane** (Go operator): Reconciles `AutoInstrumentation` CRs into OpenTelemetry `Instrumentation` resources and patches target workloads
...
When an `AutoInstrumentation` CR is applied:
